...
(function() {
  if (window.location.pathname.match(/cards\/new/)) {
    $(function() {
      var $entryList, addEntryToList, clearSelection, cloneAccordionGroupForEntry, getAccordionGroupForEntry, hiddenFieldForEntry, removeEntryFromList, removeHiddenFieldForEntry;
      $entryList = $('#expressions.accordion');
      $(document).on('click', '#yes-card', function() {
        $('#prompt').slideUp();
        return $('form#new_card').slideDown();
      });
      $(document).on('click', '#expressions.accordion .btn.use-entry', function(evt) {
        var entry, entryBtn, entrySlug, selectedEntry;
        ...
        return entryBtn.removeClass('btn-success').removeClass('active');
      });
      $('.entry-select.accordion .btn.use-entry').on('click', function(evt) {
        var entryId;
        $(this).toggleClass('btn-success');
        clearSelection();
        entryId = $(this).data('id');
        if ($(this).hasClass('btn-success')) {
          return addEntryToList(entryId);
        } else {
          return removeEntryFromList(entryId);
        }
      });
      $('.btn.use-all').on('click', function(evt) {
        var entryBtns, modal, uncheckedEntryBtns;
        modal = $(this).closest('.modal');
        entryBtns = modal.find('.use-entry.btn');
        uncheckedEntryBtns = _.reject(entryBtns, function(btn) {
          return $(btn).hasClass('btn-success');
        });
        return _.each(uncheckedEntryBtns, function(btn) {
          return $(btn).trigger('click');
        });
      });
      $('span.char').on('click', function(evt) {
        return window.charNum = $(this).index('span.char') + 1;
      });
      addEntryToList = function(entryId) {
        var entry, input;
      ...
      removeEntryFromList = function(entryId) {
        var entry;
        entry = $entryList.find(".accordion-entry" + entryId);
        return entry.remove();
      };
      getAccordionGroupForEntry = function(entryId) {
        return $(".entry-select.accordion .accordion-entry" + entryId);
      };
      cloneAccordionGroupForEntry = function(entryId) {
        return getAccordionGroupForEntry(entryId).clone();
      };
      hiddenFieldForEntry = function(entryId) {
        var input;
        return input = $('<input>', {
          type: 'hidden',
      ...
      removeHiddenFieldForEntry = function(entryId) {
        var id, input;
